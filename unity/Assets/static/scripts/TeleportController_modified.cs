...
public class TeleportController_modified: MonoBehaviour
{
    ...
    private void StepMotion(Vector2 move)
    {
        float speed = 0.1f;
        Vector3 Forward = eyeTransform.forward;
        Vector3 Right = eyeTransform.right;
        Right.y = 0;
        Forward.y = 0;
        Vector3 translation = Forward * move.y * speed + Right * move.x * speed;
        cameraRigTransform.position += translation;
    ...
    private void Teleport()
    {
        ...
        if (!TriggerIsPressed() && shouldTeleport)
        {
